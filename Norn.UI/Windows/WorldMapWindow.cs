...
internal sealed class WorldMapWindow : Window
{
    ...
    private readonly WorldMapDto _map;
    ...
    private WorldMapWindow(WorldDto world, WorldMapDto map)
    {
        ...
        var includeOthers = new CheckBox
        {
            Content = "Show explored by others",
            IsChecked = AppStateStore.Current.WorldMapShowExploredByOthers,
            Margin = new Avalonia.Thickness(8),
        };
        includeOthers.IsCheckedChanged += (_, _) =>
        {
            AppStateStore.Current.WorldMapShowExploredByOthers = includeOthers.IsChecked == true;
            AppStateStore.Save();
            UpdateRender(includeOthers.IsChecked == true);
        };
        ...
        DockPanel.SetDock(includeOthers, Dock.Left);
        toggleRow.Children.Add(includeOthers);
    ...
    internal static async Task Open(Window owner, WorldDto world, WorldMapDto map)
    {
        var window = new WorldMapWindow(world, map);
